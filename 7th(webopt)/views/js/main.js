...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
      var bhav = document.getElementsByClassName("randomPizzaContainer");
      var dx = determineDx(bhav[0],size);
       
      var newwidth = (bhav[0].offsetWidth + dx) + 'px';
     
       for (var i = 0; i < bhav.length; i++) {
      bhav[i].style.width = newwidth;
    }
  }
...
var bhavs = (document.body.scrollTop / 1250);    
  for (var i = 0; i < items.length; i++) {
    var phase = Math.sin(bhavs + (i % 5));
